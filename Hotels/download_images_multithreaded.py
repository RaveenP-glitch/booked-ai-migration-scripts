#!/usr/bin/env python3
"""
Multithreaded image downloader for Hotels images
Downloads images from URLs and saves to assets directory with progress tracking
"""
...
def download_all_images_multithreaded(url_file, assets_dir, max_workers=MAX_WORKERS):
    """
...
def main():
    ...
    print("="*70)
    print("HOTELS IMAGES DOWNLOADER (MULTITHREADED)")
    print("="*70)
    ...
    try:
        successful, failed, skipped = download_all_images_multithreaded(
            url_file, assets_dir, max_workers=MAX_WORKERS
        )
        ...
        successful, failed, skipped, _ = stats.get_stats() if 'stats' in locals() else (0, 0, 0, [])
        print(f"Downloaded: {successful}, Failed: {failed}, Skipped: {skipped}")
        sys.exit(130)
    except Exception as e:
        print(f"\n✗ Error: {e}")
        import traceback
